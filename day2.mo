...
// Debug.print(Nat.toText(student1.id) #student1.name
#student1.course); 
// // Type.toType(value)
...
stable var transactions : [Transactions] = [];
...
public func wuthdrawlBalance(amount: Float) : async Text{
 if (amount > balance){
  ...
  balance := balance - amount;
var message = "You withdraw: " Float.toText(amount);
recordTransactions(message);
return message;
}
};
